...
import com.wisnuwahyudi.aplikasikuis.databinding.FragmentSoalTwoBinding
...
class Soal_Two_Fragment : Fragment() {


    private lateinit var binding : FragmentSoalTwoBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentSoalTwoBinding.inflate(layoutInflater,container,false)
        return binding.root
    }
        ...
        ConfigNetwork.getNetwork().getDataQuestion(accData?.iD?.toInt())
            ...
            .subscribe({
                if(it.isSuccess == true){
                    ...
                    binding.cbA.setOnClickListener { if(binding.cbA.isChecked){ session.siswa_two=da?.answerA} }
                    binding.cbB.setOnClickListener {if (binding.cbB.isChecked)session.siswa_two=da?.answerB }
                    binding.cbC.setOnClickListener {if (binding.cbC.isChecked)session.siswa_two=da?.answerC }
                    binding.cbD.setOnClickListener {if (binding.cbD.isChecked)session.siswa_two=da?.answerD }

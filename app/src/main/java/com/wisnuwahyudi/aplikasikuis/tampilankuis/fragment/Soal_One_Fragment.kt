...
import com.wisnuwahyudi.aplikasikuis.databinding.FragmentSoalOneBinding
...
class Soal_One_Fragment : Fragment() {

    private lateinit var binding : FragmentSoalOneBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentSoalOneBinding.inflate(layoutInflater,container,false)
        return binding.root
    }
            ...
            .subscribe({
                if(it.isSuccess == true){
                   val da =  it?.data?.get(0)
                    ...
                    binding.cbA.setOnClickListener {if(binding.cbA.isChecked){ session.siswa_one=da?.answerA}}
                    binding.cbB.setOnClickListener {if (binding.cbB.isChecked)session.siswa_one=da?.answerB }
                    binding.cbC.setOnClickListener {if (binding.cbC.isChecked)session.siswa_one=da?.answerC }
                    binding.cbD.setOnClickListener {if (binding.cbD.isChecked)session.siswa_one=da?.answerD }
